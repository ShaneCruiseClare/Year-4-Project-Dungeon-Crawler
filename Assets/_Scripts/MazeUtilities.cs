...
namespace Project1
{
    public class MazeUtilities
    {
        ...
        public static List<MapLocation> FindAllExits(Maze maze, byte typeToFine = TileType.EXIT)
        {
            Debug.Log(" ======================== searching for exits");
            List<MapLocation> exitLocations = new List<MapLocation>();
            for (int z = 0; z < maze.depth; z++)
            {
                for (int x = 0; x < maze.width; x++)
                {
                    ...
                    if (tile == typeToFine)
                    {
                        Debug.Log("**************  exit found z (row?) = " + z + ", x (column?) = " + x);
                        exitLocations.Add(new MapLocation(x, z));
                    }
                }
            }
        ...
        public static string ExitLocationsAsString(List<MapLocation> exits)
        {
            string exitLocations = "";
            foreach (MapLocation location in exits)
            {
                exitLocations += location.ToString() + ", ";
            }
            return exitLocations;
        }
    }
